...
ImapParser = exports.ImapParser = (type) ->
...
ImapParser.prototype.reinitialize = (type) ->
  @buffers = []
  @buffer_length = 0
  @values = []
  @parser.reinitialize(type)

ImapParser.prototype.execute = (b) ->
  @parser.execute b, 0, b.length

ImapParser.prototype._modifiedUtf7ToUtf8 = (data) ->
  result = ''
  start = -1
...
ImapParser.prototype.onParserStart = (type) ->
...
ImapParser.prototype.onParserDone = (type) ->
  v = @values.pop()
  o = []
  switch type
...
ImapParser.prototype.zip = (keys, vals) ->
  o = {}
  vl = vals.length
...
ImapParser.prototype.onParserData = (b, start, len, type) ->
...
for i in ['PARSER_GREETING', 'PARSER_RESPONSE', 'PARSER_COMMAND']
  ImapParser[i.replace(/PARSER_/, '')] = ipn[i]
