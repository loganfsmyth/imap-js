(function() {
  ...
  exports.ImapParser = ImapParser = (function() {
    ...
    function ImapParser(type) {
      ...
      this.parser.onData = function(b, start, len, type) {
        return _this.onParserData(b, start, len, type);
      };
      this.parser.onStart = function(type) {
        return _this.onParserStart(type);
      };
      this.parser.onDone = function(type) {
        return _this.onParserDone(type);
      };
    }
    ...
    ImapParser.prototype.zip = function(keys, vals) {
      ...
      for (i = 0, _ref2 = keys.length; 0 <= _ref2 ? i < _ref2 : i > _ref2; 0 <= _ref2 ? i++ : i--) {
        if (typeof keys[i] !== 'undefined' && i < vl) o[keys[i]] = vals[i];
      }
      return o;
    };
    ...
    ImapParser.prototype.onParserData = function(b, start, len, type) {
      ...
      if (this.buffers.length > 0) {
        ...
        if (len > 0) b.copy(data, pos, start, start + len);
        this.buffers = [];
        this.buffer_length = 0;
      } else {
        data = b.slice(start, start + len);
      }
      if (type === ipn.IMAP_LITERAL_SIZE) return;
      value = '';
      switch (type) {
        case ipn.IMAP_ATOM:
