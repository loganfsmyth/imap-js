...
function stateDecorator(state, func) {
  return function() {
    if (this.state == state) {
      ...
      throw new Error("This command is not available in the " + stateStr(state) + " state.");
    }
  }
}
...
//  this._processUntagged(response);
  if (this.responseCallbacks[response.tag]) {
    console.log(this.untagged);
    this.responseCallbacks[response.tag].call(this, (response.type != 'OK')?response.type:null, response['resp-text']);
