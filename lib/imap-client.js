(function() {
  var EventEmitter, ImapClient, ImapParser, STATE_AUTH, STATE_ERROR, STATE_LOGOUT, STATE_SELECT, STATE_UNAUTH, defineCommand, getCommandTag, i, net, pipe, stateStr, tagChars, tls, util;
  ...
  exports.ImapClient = ImapClient = (function() {
    ...
    function ImapClient(host, port, secure, options, cb) {
      var greeted, timeout;
      ...
      if (secure === 'ssl') {
        this.stream = tls.connect(port, host, __bind(function() {
        ...
        this.stream = this.socket = net.createConnection(port, host);
      }
      this.stream.on('data', __bind(function(d) {
        return this._onData(d);
      }, this));
      this.socket.setTimeout(timeout, __bind(function() {
        if (!greeted) {
          cb(true);
        }
    ...
    ImapClient.prototype.starttls = defineCommand({
      ...
      response: function(err, resp, cb) {
        ...
        listeners = this.stream.listeners('data');
        this.stream.removeAllListeners('data');
        this.stream = pipe(pair, this.stream);
