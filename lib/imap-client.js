  ...
  */
  exports.ImapClient = ImapClient = (function() {
    ...
    ImapClient.prototype._processUntagged = function(response) {
      var _base, _base2, _base3, _base4, _base5, _name, _ref, _ref2, _ref3, _ref4, _ref5;
      switch (response.type) {
        case 'CAPABILITY':
          return this.untagged['capability'] = response.value;
        case 'LIST':
          return ((_ref = (_base = this.untagged)['list']) != null ? _ref : _base['list'] = [])[response.mailbox] = {
            path: response.mailbox.split(response.delim),
            flags: response['list-flags']
          };
        case 'LSUB':
          return ((_ref2 = (_base2 = this.untagged)['lsub']) != null ? _ref2 : _base2['lsub'] = [])[response.mailbox] = {
            path: response.mailbox.split(response.delim),
            flags: response['list-flags']
          };
        case 'STATUS':
          return this.untagged['status'] = response;
        case 'EXPUNGE':
          return ((_ref3 = (_base3 = this.untagged)['expunge']) != null ? _ref3 : _base3['expunge'] = []).push(response.value);
        case 'SEARCH':
          return this.untagged['search'] = response.value;
        case 'FLAGS':
        ...
        case 'FETCH':
          return ((_ref4 = (_base4 = this.untagged)['fetch']) != null ? _ref4 : _base4['fetch'] = {})[response.value] = response['msg-att'];
        case 'OK':
        case 'BAD':
        case 'PREAUTH':
          return ((_ref5 = (_base5 = this.untagged)[_name = response.type.toLowerCase()]) != null ? _ref5 : _base5[_name] = {})[response.text.code.type.toLowerCase()] = response.text;
      }
    };
    ...
    ImapClient.prototype.capability = defineCommand({
      ...
      response: function(err, resp, cb) {
        resp.capability = this.untagged['capability'];
        return cb(err, resp);
      }
    });
    ...
    ImapClient.prototype.select = defineCommand({
      ...
      response: function(err, resp, cb) {
        ...
        resp.flags = this.untagged['flags'];
        resp.exists = this.untagged['exists'];
        resp.recent = this.untagged['recent'];
        return cb(err, resp);
      }
    });
    ...
    ImapClient.prototype.status = defineCommand({
      ...
      response: function(err, resp, cb) {
        resp.status = this.untagged['status'];
        return cb(err, resp);
      }
    });
    ...
    ImapClient.prototype.expunge = defineCommand({
      ...
      command: "EXPUNGE",
      response: function(err, resp, cb) {
        resp.expunge = this.untagged['expunge'];
        return cb(err, resp);
      }
    });
    ImapClient.prototype.search = defineCommand({
      ...
      command: function(charset, criteria, uid) {
        return (uid ? 'UID ' : '') + ("SEARCH " + charset + " " + criteria);
      },
      response: function(err, resp, cb) {
        resp.search = this.untagged['search'];
        return cb(err, resp);
      }
    });
    ImapClient.prototype.fetch = defineCommand({
      ...
      response: function(err, resp, cb) {
        resp.fetch = this.untagged['fetch'];
        return cb(err, resp);
      }
    });
