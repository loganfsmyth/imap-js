(function() {
  ...
  STATE_LOGOUT = 0x8;
  tagChars = (function() {
    var _i, _j, _ref, _ref2, _ref3, _results, _results2, _results3;
    _results = [];
    for (i = 0x1; 0x1 <= 0x7E ? i <= 0x7E : i >= 0x7E; 0x1 <= 0x7E ? i++ : i--) {
      if (((_ref = String.fromCharCode(i)) !== '(' && _ref !== ')' && _ref !== '{' && _ref !== ' ' && _ref !== '\\' && _ref !== '"' && _ref !== '%' && _ref !== '*' && _ref !== '+') && ((_ref2 = String.fromCharCode(i), __indexOf.call((function() {
        _results2 = [];
        for (var _i = 'a'; 'a' <= 'z' ? _i <= 'z' : _i >= 'z'; 'a' <= 'z' ? _i++ : _i--){ _results2.push(_i); }
        return _results2;
      }).apply(this, arguments), _ref2) >= 0) || (_ref3 = String.fromCharCode(i), __indexOf.call((function() {
        _results3 = [];
        for (var _j = 'A'; 'A' <= 'Z' ? _j <= 'Z' : _j >= 'Z'; 'A' <= 'Z' ? _j++ : _j--){ _results3.push(_j); }
        return _results3;
      }).apply(this, arguments), _ref3) >= 0))) {
  ...
  defineCommand = function(state, _arg) {
    var command_cb, continue_cb, response_cb, states;
    command_cb = _arg.command, response_cb = _arg.response, continue_cb = _arg["continue"];
    states = [STATE_ERROR, STATE_LOGOUT, STATE_UNAUTH, STATE_AUTH, STATE_SELECT];
    return function() {
      var args, cb, command, stateName, _i;
      ...
      command = {
        ...
        response: function() {
          ...
          if (response_cb) {
            ...
            return cb.apply(null, resp_args);
          }
        }
      };
      if (continue_cb) {
        command.continuation = function() {
          var cont_args;
  ...
  ImapClient = exports.ImapClient = function(host, port, secure, cb) {
  ...
  ImapClient.prototype.enqueueCommand = function(command) {
    ...
    return console.log(tag + ' ' + command.command);
  };
