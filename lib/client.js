(function() {
  ...
  module.exports = Client = (function(_super) {
    ...
    function Client(options) {
      ...
      if (Client.debug) {
        this._con.on('data', function(c) {
          return console.error(c.toString('utf8'));
        });
      }
    ...
    Client.prototype._onUntagged = function(resp) {
      var code, k, type, v, val, value, _base, _base2, _base3, _base4, _base5, _base6, _name, _ref, _ref2, _results;
      ...
      switch (type) {
        ...
        case "FETCH":
          ...
          this._response.fetch[resp.id] = resp.value;
          _ref2 = resp.value;
          _results = [];
          for (k in _ref2) {
            ...
            v = _ref2[k];
            delete resp.value[k];
            _results.push(resp.value[k.toLowerCase()] = v);
          }
          return _results;
          break;
        default:
    ...
    Client.prototype._handleCommand = function(_arg, args, cb) {
      ...
      if (Client.debug) console.error(command);
      this._respCallbacks[t] = !response ? function(err, resp) {
        return cb(err, null, resp);
      } : function(err, resp) {
        return response.call(_this, err, resp, cb, args);
      };
      if (cont) {
    ...
    Client.prototype.status = cmd({
      ...
      response: function(err, resp, cb, args) {
        var item_names, mailbox;
        mailbox = args[0], item_names = args[1];
        return cb(err, resp.status[mailbox], resp);
      }
    });
    ...
    Client.prototype.search = cmd({
      command: function(crit, uid) {
        return (uid && 'UID ' || '') + 'SEARCH CHARSET UTF-8 ' + this._searchCriteria(crit);
      },
      response: function(err, resp, cb) {
    ...
    Client.prototype.fetch = cmd({
      command: function(seq, crit, uid) {
        var com;
        if (Array.isArray(seq)) seq = seq.join(',');
        seq = ('' + seq).replace(' ', '');
        com = (uid && 'UID ' || '') + "FETCH " + seq;
    ...
    Client.prototype.store = cmd({
      command: function(seq, op, flags, uid) {
        var com;
        if (Array.isArray(seq)) seq = seq.join(',');
        seq = ('' + seq).replace(' ', '');
        com = (uid && 'UID ' || '') + "STORE " + seq + ' ';
        com += (function() {
          switch (op) {
            case 'add':
              return '+';
    ...
    Client.prototype.copy = cmd({
      command: function(seq, mailbox, uid) {
        if (Array.isArray(seq)) seq = seq.join(',');
        seq = ('' + seq).replace(' ', '');
        return (uid && 'UID ' || '') + "COPY " + seq + ' ' + q(mailbox);
      }
    });
