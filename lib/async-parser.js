(function() {
  ...
  date_text = function() {
    return join(series([
      starts_with(' ', series([sp(), digits(1)]), digits(2)), str('-'), onres(oneof(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']), function(result) {
  ...
  body_type_1part = function() {
    return series([body_type_1part_main(), ifset(' ', series([sp(), body_ext_1part()], 1))]);
  };
  ...
  status_att = function() {
    return oneof(['MESSAGES', 'RECENT', 'UIDNEXT', 'UIDVALIDITY', 'UNSEEN']);
  };
  ...
  search_args = function() {
    ...
    hascharset = series([str('CHARSET'), sp(), astring(), sp(), nocharset], [0, 2, 4]);
    return function() {
      var handler, i;
      i = 0;
      handler = null;
  ...
  store_att_flags = function() {
    return zip(['op', 'silent', null, 'flags'], series([oneof(['+FLAGS', '-FLAGS', 'FLAGS']), ifset('.', str('.SILENT')), sp(), starts_with('(', flag_list(), space_list(flag()))]));
  };
