(function() {
  ...
  exports.Tokenizer = Tokenizer = (function(_super) {
    ...
    Tokenizer.prototype.end = function(str, encoding) {
      if (!this.writable) return;
      if (str) this.write(str, encoding);
      return this.destroySoon();
    };

    Tokenizer.prototype.destroySoon = function() {
      this.writable = false;
      if (!this.writing && this.token) {
        this.emit('token', {
          type: this.token | exports.TOKEN_END,
          data: new Buffer(0)
        });
        this.token = null;
      }
      if (!this.writing) return this.destroy();
    };
    ...
    Tokenizer.prototype.write = function(buffer, encoding) {
      ...
      pos = 0;
      this.writing = true;
      while (!this.destroyed && pos < buffer.length) {
    ...
    Tokenizer.prototype.syntaxError = function(buffer, pos) {
      var c, end, err, prefix, start, suffix;
      end = Math.min(pos + 10, buffer.length);
      start = Math.max(0, pos - 10);
      prefix = buffer.slice(start, pos);
      suffix = pos + 1 === buffer.length && buffer.slice(pos + 1, end) || '';
      c = buffer.slice(pos, (pos + 1)).toString().replace(/\r/, function(c) {
        var code;
        code = c.charCodeAt(0);
        return '0x' + ((code < 16) && '0' || '') + code.toString(16);
      });
